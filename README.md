...
# 추가 로그 소스 설정
```
dnf/php_error/audit/tallylog/btmp/wtmp/firewalld/chrony
```
...
# tallylog/btmp/wtmp
```
tail 명령어가 안 막히는 바이너리 로그는 faillock / lastb / last 명령을 30초 간격으로 실행해 이전 결과와 비교(diff)하는 방식으로 동작 (opt/log_client/main.py의 run_polling_source)
```
